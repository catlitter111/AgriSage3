...
class StereoCamera:
    ...
    def __init__(self, camera_id=0, width=1280, height=480):
        """
        初始化双目相机
    ...
    def load_camera_params(self, file_path=None):
        ...
        if file_path and os.path.exists(file_path):
            try:
                ...
                # 使用硬编码的相机参数
                self._use_hardcoded_params()
        else:
            # 使用硬编码的相机参数
            self._use_hardcoded_params()
        
        # 返回参数供其他方法使用
        return self.left_camera_matrix, self.left_distortion, self.right_camera_matrix, self.right_distortion, self.R, self.T
        
    def _use_hardcoded_params(self):
        """使用硬编码的相机参数"""
        # 从原始代码中提取的硬编码参数
        ...
        self.left_distortion = np.array([
            [0.0544639674308284, -0.0266591889115199, 0.00955609439715649, -0.0026033932373644, 0]
        ])
        ...
        self.right_distortion = np.array([
            [0.069434162778783, -0.115882071309996, 0.00979426351016958, -0.000953149415242267, 0]
        ])
        self.R = np.array([
