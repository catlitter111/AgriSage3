...
def generate_launch_description():
    ...
    # 异步处理参数
    thread_num_arg = DeclareLaunchArgument(
        'thread_num',
        default_value='3',
        description='异步处理线程数量'
    )
    
    queue_size_arg = DeclareLaunchArgument(
        'queue_size',
        default_value='10',
        description='异步处理队列大小'
    )
    
    # 创建异步瓶子检测节点
    bottle_detection_node = Node(
        package='bottle_detection_ros2',
        executable='bottle_detection_node_async',
        name='bottle_detection_node_async',
        output='screen',
        emulate_tty=True,
        parameters=[{
    ...
    log_info = LogInfo(
        msg=['正在启动异步瓶子检测节点...\n',
             ...
             '线程数: ', LaunchConfiguration('thread_num'), '\n',
             '队列大小: ', LaunchConfiguration('queue_size'), '\n',
